    ...
    public function insertUser()
{
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!empty($errors)) {
            ...
            $result = $uDAO->insertUser($nombre, $email, $contrasena, $rol, $telefono, $direccion, $fecha_registro);
    ...
    public function validacioniniciosesion() {
        ...
        if (isset($_POST['iniciarsesion'])) {
            ...
            if (!isset($_POST['contrasena']) || strlen(trim($_POST['contrasena'])) == 0) {
                $errores['contrasena'] = "La contraseña no puede estar vacía.";
            }
            
            if (empty($errores)) {
                $uDAO = new UserDAO();
                $user = $uDAO->getAllUsers($_POST['nombre'], $_POST['contrasena']); // Suponiendo un método adecuado en el DAO
                
                if (empty($user)) {
                    $errores['general'] = "El usuario no está registrado.";
                    View::show("iniciarsesion", $errores);
                } else {
                    // Asigna valores a la sesión desde el resultado del DAO
                    $_SESSION['id_usuario'] = $user['id_usuario'];
                    $_SESSION['nombre'] = $user['nombre'];
                    $_SESSION['rol'] = $user['rol']; // Asumiendo que el rol está en el campo 'rol' del usuario
                    
                    if ($user['rol'] == 'administrador') {
                        View::show("paginaUsuarios", array('rol' => 'administrador'));
                    } else {
                        View::show("paginaUsuarios", array('rol' => 'normal'));
                    }
                }
            } else {
                View::show("iniciarSesion", $errores);
